...
class DataConnection implements Runnable {
    ...
    public void run() {
        try {
            ...
            if (this.request.getVerb().equalsIgnoreCase("upid")) {
                int upload_id = (Integer) this.request.getBody().get("upload_id");
                try {
            ...
            if (this.upload_metadata.get_filesize() == total_chunksize) {
                ...
                System.out.println("dc: success upload.");
    ...
    Request get_request() {
        ...
        if (request.getVerb().equalsIgnoreCase("timeout")) {
            this.cancel_upload();
        }
        return this.request;
    }
